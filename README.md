...
参考「账套管理模式」(账套 = 租户),三层主体职责清晰分层、互不越界:

| 维度 | 平台超级管理员 | 账套管理员(租户管理员) | 普通用户 |
|---|:---:|:---:|:---:|
| 定位 | 系统级运维 | 本账套内的超管 | 按授权操作 |
| 开通/停用账套、套餐与到期 | ✅ | ❌ | ❌ |
| 跨账套访问、平台运维、管理其它超管 | ✅ | ❌ | ❌ |
...
- **平台超级管理员**(`User.is_super_admin`,全局用户):`user_has()` 恒放行;免成员/订阅校验;可登录任意租户。平台管理端点(`/api/platform/*`,含跨租户用户列表与设/取消超管——保留至少一个)与运行诊断(`/api/diag/*`)仅其可访问。
- **账套管理员**(`TenantMembership.is_tenant_admin`):由 `auth_mw` 按当前租户识别并挂到请求级用户;`user_has()` 对**本租户所有模块**豁免(含用户与授权管理),`me`/登录返回 `permissions: ["*"]` 使前端菜单一致。边界由令牌 `tid` + 成员校验保证:不可跨账套、不可访问平台/诊断、不可创建平台超管、受订阅到期约束。
- **普通用户**:RBAC 角色 × 权限点(`module:action`,`module:*` 展开为该模块全部动作),角色与权限按租户隔离。
- **私有化模式**:`is_saas()` 门控上述多租户逻辑,私有化下不启用账套管理员豁免,行为与单机版一致。
