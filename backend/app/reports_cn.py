...
def _asset_lines(b: Balances) -> dict[int, Decimal]:
    ...
    # 存货含在产品(生产成本 5001/制造费用 5101/劳务成本 5201 期末余额),
    # 以保证成本类科目残额进入资产方,资产 = 负债 + 所有者权益 恒等成立
    v[9] = b.net_debit("1401", "1403", "1404", "1405", "1406",
                        "1407", "1408", "1411", "1471",
                        "5001", "5101", "5201")             # 存货
    v[10] = b.net_debit("1403")                            # 其中:原材料
    v[11] = b.net_debit("5001", "5101", "5201")            # 在产品(生产成本等)
...
_ASSET_TEMPLATE = [
    ...
    {"label": "其中:原材料", "line": 10, "style": "subitem", "indent": 1},
    {"label": "在产品", "line": 11, "style": "subitem", "indent": 2},
    {"label": "库存商品", "line": 12, "style": "subitem", "indent": 2},
    {"label": "周转材料", "line": 13, "style": "subitem", "indent": 2},
...
_INCOME_TEMPLATE = [
    ...
    ("资源税", 7, "sub", 2),
    ("土地增值税", 8, "sub", 2),
    ("城镇土地使用税、房产税、车船税、印花税", 9, "sub", 2),
    ("教育费附加、矿产资源补偿费、排污费", 10, "sub", 2),
    ("销售费用", 11, "item", 0),
    ("其中:商品维修费", 12, "sub", 1),
    ("广告费和业务宣传费", 13, "sub", 2),
    ("管理费用", 14, "item", 0),
    ...
    ("减:营业外支出", 24, "item", 0),
    ("其中:坏账损失", 25, "sub", 1),
    ("无法收回的长期债券投资损失", 26, "sub", 2),
    ("无法收回的长期股权投资损失", 27, "sub", 2),
    ("自然灾害等不可抗力因素造成的损失", 28, "sub", 2),
    ("税收滞纳金", 29, "sub", 2),
    ("三、利润总额(亏损总额以“-”号填列)", 30, "head", 0),
    ("减:所得税费用", 31, "item", 0),
    ("四、净利润(净亏损以“-”号填列)", 32, "head", 0),
]
...
def _cashflow_values(db: Session, start: date, end: date,
    ...
    for vid in voucher_ids:
        ...
        # 按对方分录所在借贷方向归类(而非金额符号),使红字冲销自动抵减同一行:
        # 贷方分录 → 现金流入类行(收到);借方分录 → 现金流出类行(支付)。
        # 金额保留正负号,冲销(负数)保持原方向,故与原分录落在同一行相互抵减。
        for e, a in rows:
            if a.code in CASH_CODES:
                continue
            if e.credit != 0:
                v[_CF_INFLOW.get(a.code, 2)] += e.credit
            if e.debit != 0:
                v[_CF_OUTFLOW.get(a.code, 6)] += e.debit
...
# (label, line, style, indent)
_CASHFLOW_TEMPLATE = [
    ("一、经营活动产生的现金流量:", None, "head", 0),
    ("销售产成品、商品、提供劳务收到的现金", 1, "item", 1),
    ("收到其他与经营活动有关的现金", 2, "item", 1),
    ("购买原材料、商品、接受劳务支付的现金", 3, "item", 1),
    ("支付的职工薪酬", 4, "item", 1),
    ("支付的税费", 5, "item", 1),
    ("支付其他与经营活动有关的现金", 6, "item", 1),
    ("经营活动产生的现金流量净额", 7, "total", 1),
    ("二、投资活动产生的现金流量:", None, "head", 0),
    ("收回短期投资、长期债券投资和长期股权投资收到的现金", 8, "item", 1),
    ("取得投资收益收到的现金", 9, "item", 1),
    ("处置固定资产、无形资产和其他非流动资产收回的现金净额", 10, "item", 1),
    ("短期投资、长期债券投资和长期股权投资支付的现金", 11, "item", 1),
    ("购建固定资产、无形资产和其他非流动资产支付的现金", 12, "item", 1),
    ("投资活动产生的现金流量净额", 13, "total", 1),
    ("三、筹资活动产生的现金流量:", None, "head", 0),
    ("取得借款收到的现金", 14, "item", 1),
    ("吸收投资者投资收到的现金", 15, "item", 1),
    ("偿还借款本金支付的现金", 16, "item", 1),
    ("偿还借款利息支付的现金", 17, "item", 1),
    ("分配利润支付的现金", 18, "item", 1),
    ("筹资活动产生的现金流量净额", 19, "total", 1),
    ("四、现金净增加额", 20, "head", 0),
    ("加:期初现金余额", 21, "item", 0),
    ("五、期末现金余额", 22, "head", 0),
]
...
def _assemble(period: Period, template, cur: dict[int, Decimal],
    ...
    rows = []
    for label, line, style, indent in template:
        if line is None:
            rows.append({"label": label, "line": None, "style": style,
                         "indent": indent, "col1": None, "col2": None})
        else:
            rows.append({
                "label": label, "line": line, "style": style, "indent": indent,
