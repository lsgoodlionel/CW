...
> 以下为已提出但**尚未落地**的需求规格,供接力开发。#4/#5 涉及引入登录鉴权,会改变全系统访问方式,落地前需确认「是否强制登录」。

### 7.1 流程/审批设计模块(核心可扩展)
- 目标:在专有页面可视化设计审批流程,供各业务单据复用。
- 建议模型:
  - `workflow_definitions`(流程定义:名称/业务类型/启用)
  - `workflow_steps`(步骤:顺序/审批人来源=指定员工/角色/部门负责人/条件)
  - `workflow_instances`(实例:关联单据/当前步骤/状态 draft/pending/approved/rejected)
  - `workflow_tasks`(待办:实例×步骤×审批人/结果/意见/时间)
- API:流程定义 CRUD;单据发起 `submit`;审批 `approve/reject`;我的待办列表。
- 前端:流程设计页(步骤编排)+ 待办中心 + 单据审批轨迹。
